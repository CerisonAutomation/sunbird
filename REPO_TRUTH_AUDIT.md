...
| Browser client | **PASS (local)** | `npm run verify` passes; typecheck, tests (**267** across 34 files), lint, build all clean. |
...
| PWA/offline | **PASS** | Manifest + service worker present; all icon assets exist in `public/icons/` (apple-touch, favicon-32/64, icon-192, icon-512). Cache key is `sunbird-shell-${BUILD_ID}` (`public/sw.js:5`), where `BUILD_ID` is generated per build in `vite.config.ts:26` and substituted into `dist/sw.js` at emit — so it busts automatically on every build and there is no manual version to forget to bump. (`GAME_AUDIT_2026-09.md` records a manual `sunbird-shell-v3`; that scheme has since been replaced.) |
| CI | **PASS** | Three workflows: `.github/workflows/ci.yml` (lint/typecheck/test/build + portal zips + production gate), `rust.yml` (fmt **check** + clippy + test + release), and `botsim.yml` (40 headless pilots on the real wire protocol; the Node job reports cheat containment, the Rust job gates on it with `--require-anticheat`, and both post their numbers to the PR). `rust.yml` used to run `cargo fmt` and push the result as a bot commit; that caused a push race between concurrent runs and left follow-up runs stuck in `action_required`, so rustfmt is now a failing gate instead. |
...
- `npm test` (Vitest): **267/267 pass** across 34 files (re-run at this refresh).
  The 100× stability loop below was measured when the suite was 212 tests and
  has **not** been re-run since; the flake claim applies to that older suite.
- Historical: **212/212 pass** across 31 files. 100× stability loop: 100/100
  runs pass, zero flakes.
