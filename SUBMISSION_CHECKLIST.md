...
|---|---|---|
| `poki-upload/` | 1.68 MB html + icons/fonts/i18n — **the folder the Inspector is given** | `pnpm verify:upload` |
| `sunbird-poki.zip` | 829 KB (1.72 MB html) | `pnpm verify:portals`, `pnpm audit:zips`, `pnpm verify:upload` |
...
```bash
pnpm poki:preflight    # build:portals → verify:portals → audit:zips → verify:upload → verify:thumbnail → poki:audit --run
```
...
2. Open <https://inspector.poki.dev/> and **drag the folder `poki-upload/`** onto the drop zone (the Inspector takes a folder, and `index.html` — 1.68 MB, everything inlined — sits at its root).
...
**Next action:** run `pnpm upload:poki`, then drag the generated `poki-upload/` folder onto <https://inspector.poki.dev/> and walk the QA modules.
