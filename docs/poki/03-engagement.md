...
| `EN-05` | Confetti + jingle on personal bests, level-ups, chest opens and race wins; personal-best moments also trigger the portal's own celebration hook when one exists (`happytime()` on Poki). | `src/game/Fx.ts`, `src/game/HudFeedback.ts`, `src/sdk/poki.ts` |
