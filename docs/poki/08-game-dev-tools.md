...
| `TOOL-08` | AUDS stays Poki-only **by construction**: `createAudsIfConfigured()` returns `null` unless `VITE_POKI_GAME_ID` is set, and portal/generic/itch builds ship without it, so every AUDS call is skipped and the UI says so instead of offering a dead button. `pnpm isolation:check` fails the build if `auds.poki.io` or a self-hosted endpoint appears in the wrong edition. Submission step: set `VITE_POKI_GAME_ID` (AUDS) and `VITE_POKI_NETLIB_GAME_ID` (Netlib) to the ids Poki issues. |
