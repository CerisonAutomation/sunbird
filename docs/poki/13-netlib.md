...
| `NL-08` | The client is created through a **dynamic import** in `net-transport.poki.ts`, so `@poki/netlib` is fetched at race time, not at boot — and `pnpm isolation:check` proves it is absent from every non-Poki bundle. |
