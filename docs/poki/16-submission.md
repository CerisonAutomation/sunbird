...
| `SUB-11`, `SUB-12` | Portal editions compile with `SQUAD_CHAT = false`, `CUSTOM_PILOT_NAMES = false` and no purchase path (`SELL_AD_REMOVAL = false`): no chat UI, no typed player text, no personal data, no IAP in the bundle at all. |
