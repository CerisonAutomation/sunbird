...
| `THB-09` | requirement | Static thumbnail for the player-fit test; animated 3-5 s gameplay loop before global release. | 📋 action | Record the 3-5 s capture from a real play session (the build is capture-ready: embed-safe, no fullscreen requirement). |
...
| `TOOL-03` | requirement | Uploadable as a folder with index.html at the root, working from any sub-path. | ✅ | gate wired: node scripts/verify-portal.mjs |
