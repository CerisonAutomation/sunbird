...
async function main() {
  ...
  const gates = [
    ...
    {
      // The name has to match what was actually checked. With
      // `pass: ... : true` this printed "[PASS] cheats are contained by the
      // server - 402 leaked", which reads as containment and is the opposite
      // of the truth: by default this is a MEASUREMENT, not a gate. Callers
      // scanning the gate list for green had no way to tell the difference.
      name: REQUIRE_ANTICHEAT
        ? "cheats are contained by the server"
        : leaks.length === 0
          ? "cheats contained (incidental — not gated)"
          : "cheat relay MEASURED, not gated — leaks are expected here",
  ...
  // The default target is the legacy Node room server (server/sunbird-server.mjs,
  // :8787), which has NO motion validation. The server that actually ships is
  // the Rust sunbird-server on :8080, whose validate.rs rejects teleports and
  // absurd coordinates under test. A green run against the default therefore
  // says nothing about production anticheat — say so out loud rather than
  // leaving it to whoever reads the gate list next.
  const isRustTarget = /:8080/.test(URL);
  console.log("\n— target ————————————————————————————————————————————");
  console.log(`  url         ${URL}`);
  console.log(
    isRustTarget
      ? "  server      Rust sunbird-server (has validate.rs anticheat)"
      : "  server      legacy Node room server — NO anticheat. Production runs the\n" +
          "              Rust server on :8080; re-run with --url ws://127.0.0.1:8080/ws\n" +
          "              --require-anticheat to gate the server that ships.",
  );
