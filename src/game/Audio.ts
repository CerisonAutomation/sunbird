...
export class GameAudio {
  ...
  /** Cache noise buffers for performance - prevents creating new buffers every sound */
  private noiseBufferCache = new Map<string, AudioBuffer>();
  ...
  private getNoiseBuffer(duration: number): AudioBuffer {
    ...
    // Cache up to 50 buffers to prevent memory issues
    if (this.noiseBufferCache.size > 50) {
      const firstKey = this.noiseBufferCache.keys().next().value;
      if (firstKey) this.noiseBufferCache.delete(firstKey);
    }
  ...
  dispose(): void {
    this.music?.dispose();
    // Clear noise buffer cache to free memory
  ...
  update(
    ...
    if (!this.ctx || !this.whooshGain || !this.whooshFilter || !this.windGain) return;
  ...
  perfect(): void {
    // Sub-thump punch + bright ascending chord
    this.tone(80, 0.22, "sine", 0.06, 40);      // sub thump
    this.tone(523.25, 0.22, "sine", 0.16, 524.5);
    this.tone(659.25, 0.24, "sine", 0.14, 660.5);
    this.tone(783.99, 0.28, "sine", 0.13, 785.0);
    this.tone(1046.5, 0.32, "triangle", 0.09, 1048.0);
    this.noiseBurst(0.08, 2400, 0.04);           // crack transient
  }

  /** Rising pitch chime — pentatonic scale so each combo step sounds musical. */
  comboRisingPitch(combo: number): void {
    // Pentatonic: C D E G A (repeating up 3 octaves, starting C5)
    ...
    const vel = Math.min(0.12 + combo * 0.006, 0.22);
    this.tone(base, vel, "sine", 0.09, base * 1.003);
    this.tone(base * 1.5, vel * 0.55, "triangle", 0.05);
    if (combo >= 5) this.tone(base * 2, vel * 0.3, "sine", 0.04);
  }
  ...
  /** "Wow!" — wide descending pitch sweep, bright + breathy */
  wow(): void {
    this.tone(800, 0.35, "sine", 0.13, 300);
    this.tone(1200, 0.3, "triangle", 0.06, 500);
    this.noiseBurst(0.15, 3000, 0.04);
  }

  /** "Yeah!" — sharp upward bark then release */
  yeah(): void {
    this.tone(300, 0.06, "sawtooth", 0.11, 600);
    this.tone(600, 0.22, "sine", 0.12, 440);
    this.tone(880, 0.18, "triangle", 0.05, 660);
  }

  /** "Yip!" — quick high chirp with punch */
  yip(): void {
    this.tone(900, 0.08, "sine", 0.15, 1400);
    this.tone(1400, 0.1, "triangle", 0.08, 1800);
    this.noiseBurst(0.07, 4000, 0.05);
  }

  /** "Woaaahh!" — slow rising swell, awe-struck */
  woah(): void {
    this.tone(220, 0.7, "sine", 0.1, 480);
    this.tone(330, 0.65, "triangle", 0.07, 660);
    this.tone(440, 0.6, "sine", 0.05, 880);
    this.noiseBurst(0.5, 600, 0.03);
  }

  /** Unlock celebration — ascending fanfare with shimmer */
  unlock(): void {
    // Rising arpeggio: C5 → E5 → G5 → C6
    this.tone(523.25, 0.12, "triangle", 0.1);
    setTimeout(() => this.tone(659.25, 0.12, "triangle", 0.1), 80);
    setTimeout(() => this.tone(783.99, 0.12, "triangle", 0.1), 160);
    setTimeout(() => this.tone(1046.5, 0.25, "sine", 0.12), 240);
    // Shimmer tail
    setTimeout(() => this.tone(1318.5, 0.3, "sine", 0.06), 320);
    setTimeout(() => this.tone(1567.98, 0.4, "sine", 0.04), 400);
    // Sparkle burst
    setTimeout(() => this.noiseBurst(0.15, 6000, 0.04), 280);
  }

  /** Trophy achievement — triumphant brass-like fanfare */
  trophy(): void {
    this.tone(440, 0.15, "sawtooth", 0.08);
    setTimeout(() => this.tone(554.37, 0.15, "sawtooth", 0.08), 100);
    setTimeout(() => this.tone(659.25, 0.15, "sawtooth", 0.08), 200);
    setTimeout(() => this.tone(880, 0.4, "sine", 0.12), 300);
    setTimeout(() => this.noiseBurst(0.2, 2000, 0.05), 350);
  }

  /** Season pass tier up — magical chime cascade */
  tierUp(): void {
    const notes = [880, 1108.73, 1318.51, 1760];
    notes.forEach((freq, i) => {
      setTimeout(() => this.tone(freq, 0.15, "sine", 0.08), i * 70);
    });
    setTimeout(() => this.tone(2093, 0.3, "sine", 0.06), 300);
  }
  ...
  private noiseBurst(dur: number, freq: number, gain: number): void {
    ...
    const buffer = this.getNoiseBuffer(dur);
