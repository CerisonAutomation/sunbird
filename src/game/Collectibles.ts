...
type Coin = { x: number; y: number; taken: boolean; gem: boolean; mesh: THREE.Mesh; glow?: THREE.Sprite };
...
export const PICKUP_STYLE: Record<PickupKind, { color: number; emissive: number; icon: string; label: string }> = {
  ...
  timefreeze: { color: 0xa0d8ff, emissive: 0x0040c0, icon: "❄", label: "Time Freeze" },
  doublecoins: { color: 0xffe840, emissive: 0xc08000, icon: "⚡", label: "2× Coins" },
  starburst: { color: 0xff80ff, emissive: 0x800080, icon: "🌟", label: "Star Burst" },
};
...
export class Collectibles {
  ...
  update(dt: number, bird: Bird, terrain: TerrainSystem, magnetOn: boolean, time: number, magnetScale: number, ev: CollectEvents): void {
    ...
    for (let i = this.activeCoins.length - 1; i >= 0; i--) {
      ...
      c.mesh.position.set(c.x, c.y + Math.sin(time * 5 + c.x * 0.35) * 0.35 + Math.sin(time * 3.2 + c.x * 0.7) * 0.12, 0);
      c.mesh.rotation.y += dt * (c.gem ? 1.6 : 3.2);
      c.mesh.rotation.x = Math.sin(time * 2.8 + c.x * 0.5) * 0.15;
      // Gem glow ring: pulses in size and opacity
      if (c.glow) {
        const gp = 1 + Math.sin(time * 3.5 + c.x * 0.4) * 0.2;
        c.glow.scale.setScalar(3.2 * gp);
        (c.glow.material as THREE.SpriteMaterial).opacity = 0.6 + Math.sin(time * 4.2 + c.x * 0.6) * 0.2;
      }
    }
  ...
  private allocCoin(gem: boolean): Coin {
    const idle = this.coinPool.find((c) => (c.taken || !c.mesh.visible) && c.gem === gem);
    if (idle) return idle;
    const mesh = new THREE.Mesh(gem ? this.gemGeo : this.coinGeo, gem ? this.gemMat : this.coinMat);
    if (!gem) mesh.rotation.z = Math.PI / 2;
    mesh.layers.set(this.layer);
    this.group.add(mesh);
    let glow: THREE.Sprite | undefined;
    if (gem) {
      glow = makeGemGlow();
      glow.layers.set(this.layer);
      mesh.add(glow);
    }
    const coin: Coin = { x: 0, y: 0, taken: false, gem, mesh, glow };
...
function makeGemGlow(): THREE.Sprite {
  const c = document.createElement("canvas");
  c.width = 128;
  c.height = 128;
  const g = c.getContext("2d")!;
  const grd = g.createRadialGradient(64, 64, 8, 64, 64, 64);
  grd.addColorStop(0, "rgba(154,232,255,0.7)");
  grd.addColorStop(0.35, "rgba(120,200,240,0.3)");
  grd.addColorStop(0.65, "rgba(100,180,220,0.1)");
