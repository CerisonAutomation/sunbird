...
export type GoalKind = "distance" | "perfects" | "combo" | "altitude" | "coins" | "clouds" | "gems" | "zeniths" | "islands" | "fever";
...
const GOAL_SHAPES: { kind: GoalKind; base: number; scale: number; word: (n: number) => string }[] = [
  ...
  { kind: "zeniths", base: 1, scale: 3, word: (n) => `Hit ${n} zenith moment${n > 1 ? "s" : ""}` },
  { kind: "islands", base: 1, scale: 2, word: (n) => `Reach island ${n + 1}` },
  { kind: "fever", base: 1, scale: 2, word: (n) => `Trigger fever ${n} time${n > 1 ? "s" : ""}` },
];
