export class Input {
  ...
  get diving2(): boolean {
    return this.p2Key || this.p2Touch || this.padP2;
  }

  /** Set dive state from voice control. */
  setDiv(diving: boolean): void {
    this.held = diving;
  }
  ...
  private f3Pressed = false;

  consumeF3(): boolean {
    const v = this.f3Pressed;
  ...
  chatCode: string | null = null;

  consumeChat(): string | null {
    const c = this.chatCode;
    this.chatCode = null;
    return c;
  }
  ...
  private onKeyDown(e: KeyboardEvent): void {
    ...
    if (e.code === "Space") {
      ...
      e.preventDefault(); // Prevent Escape from closing fullscreen (CrazyGames QA)
