...
export class LaunchSystem {
  ...
  evaluate(bird: Bird, terrain: TerrainSystem, time: number): LaunchResult {
    ...
    if (speed < LAUNCH_MIN_SPEED || angleOut <= 0.04) {
    ...
    const since = this.releaseAt >= 0 ? time - this.releaseAt : 99;
    const fRelease = since > 90 ? 0.08 : 1 - clamp(since / LAUNCH_RELEASE_WINDOW, 0, 1) * 0.88;
