...
export const ONBOARDING_TIPS: Record<OnboardingId, Omit<OnboardingTip, "id">> = {
  ...
  "store-birds": {
    title: "Birds = your playstyle (million times better)",
    body: "Every bird changes HOW you fly: Speed birds (Phoenix) = faster dive, Fever birds (Aurora) = easier warp, Daylight birds (Owl) = longer runs, Magnet birds (Bluejay) = coins pull from 15m. Starter free, rare = perks, mythic = game-changers. Research: best players pick bird for circuit — Reef = magnet, Volcano = shield, Canyon = speed. Try all!",
  ...
  "store-upgrades": {
    title: "Nest = permanent progress (best investment)",
    body: "Nest upgrades = ONLY permanent coin multiplier. Lv.1 = ×1.12, each +12%, max ×2.2. Costs coins ONCE, pays FOREVER. Unlike birds (situational), nest works everywhere. Research: players who upgrade nest first earn 3× more coins by day 7. Best long-term, not short-term. Buy before mythic birds!",
    target: ".boost-row",
    action: "open-shop-upgrades",
    priority: 60,
  },
  "pvp-online": {
    title: "PvP = real humans, real bragging (40 max)",
    body: "PvP = online races vs real pilots. Quick match = instant search, fills with AI if nobody answers (no waiting). Private rooms = custom code + link, invite friends via #room=CODE or portal invites. 40 max per room = massive flock. Same physics as solo, but slipstream drafting + slingshot + wingtip buzz = new skills. Fair race disables paid boosts — skill only. Research: best retention comes from social — race friends!",
  ...
  "pvp-ranked": {
    ...
    body: "Ranked = rating that climbs: Fledgling (0) → Soarer (400) → Glider (800) → Sky Racer (1200) → Wind Master (1600) → Storm Chaser (2000) → Legendary (2600). Win = +rating, lose = -rating but never below 0. Fair race = no paid boosts, only skill. Seasons reset monthly — top divisions get Gold + exclusive trails. Research: visible progress > hidden — you SEE rating bar fill, creates obligation to finish. One more game!",
  ...
  "pve-ai": {
    ...
    body: "AI PvP = offline race vs neural flock — same physics, no internet, instant start. AI uses real downslope timing, slipstream drafting, slingshot attacks — they learn YOUR lines. Pick 5/10/20/40 opponents, Chill/Sharp/Ace skill. Great for learning: Reef = practice thermals, Volcano = practice storm dodge, Canyon = practice monster launches. Research: Tiny Wings island-by-island — practice one circuit until perfect, then race humans. Best players do 10 AI races before ranked.",
