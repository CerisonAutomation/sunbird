import { SEASON_TIERS, SEASON_XP_PER_TIER, seasonId } from "./constants";
export { seasonId }; // Re-export for backward compatibility
...
export class SeasonPass {
  ...
  /** Days remaining until the season resets (end of current month). */
  getDaysUntilReset(): number {
    const now = new Date();
    const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59, 999);
    const msLeft = endOfMonth.getTime() - Date.now();
    return Math.max(0, Math.ceil(msLeft / 86_400_000));
  }

  /** 0-1 representing how far through the current season (month) we are. */
  getSeasonProgress(): number {
    const now = new Date();
    const startOfMonth = new Date(now.getFullYear(), now.getMonth(), 1);
    const endOfMonth = new Date(now.getFullYear(), now.getMonth() + 1, 0, 23, 59, 59, 999);
    const total = endOfMonth.getTime() - startOfMonth.getTime();
    const elapsed = Date.now() - startOfMonth.getTime();
