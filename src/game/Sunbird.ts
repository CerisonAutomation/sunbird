...
/** Per-channel multiply, clamped. Used to shade a plumage colour. */
function shade(rgb: [number, number, number], f: [number, number, number]): string {
  return (
    "#" +
    rgb
      .map((v, i) => Math.max(0, Math.min(255, Math.round(v * f[i]!))))
...
/**
 * Build a full 10-slot palette from a shop skin's four colours.
 *
 * The shop used to fake each bird out of three CSS divs — an ellipse with an
 * `inset box-shadow` standing in for a belly — so sixty-odd skins rendered as
 * blobs while the rest of the game drew the real mark. The multiplier triples
 * below are read straight off SUNBIRD_PALETTE (wingFar/wingNear = .78/.53/.41,
 * tail = .88/.87/.77 of body, and so on), so a derived skin shades exactly the
 * way the reference bird does instead of by a guess that happens to look fine
 * on one colour.
 */
export function skinPalette(skin: {
  ...
  const body = rgbOf(skin.body);
  const wing = rgbOf(skin.wing);
  return {
    body: hexOf(skin.body),
    belly: hexOf(skin.belly),
    wingNear: hexOf(skin.wing),
    wingFar: shade(wing, [0.78, 0.53, 0.41]),
    tail: shade(body, [0.88, 0.87, 0.77]),
    tailTip: shade(body, [0.75, 0.59, 0.52]),
    brow: shade(body, [0.85, 0.61, 0.67]),
    beak: hexOf(skin.beak),
    // Dark, but faintly tinted by the plumage so the eye belongs to the bird.
    eye: shade(body, [0.17, 0.15, 0.2]),
    eyeWhite: "#ffffff",
  };
}
