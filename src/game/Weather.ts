...
export class Weather {
  ...
  inThermal = false;
  lightningFlash = 0; // 0..1 lightning flash intensity (decays each frame)
  ...
  private readonly RAIN_COUNT = 120;
  // Wind streak particles
  private readonly windPoints: THREE.Points;
  private readonly windPos: Float32Array;
  private readonly windVel: Float32Array;
  private readonly windMat: THREE.ShaderMaterial;
  private readonly WIND_COUNT = 60;
  ...
  constructor(private readonly seedN: number) {
    ...
    // Rain particles — small blue lines falling
    ...
    this.rainMat = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          ...
          gl_PointSize = 3.0 * (220.0 / max(1.0, -mv.z));
      ...
      fragmentShader: `
        ...
        void main() {
          ...
          float d = length(p * vec2(1.0, 0.3));
          if (d > 0.5) discard;
          float a = smoothstep(0.5, 0.1, d) * vAlpha;
          gl_FragColor = vec4(0.5, 0.7, 1.0, a * 0.6);
        }
      `,
    });
    ...
    // Init rain drops at random positions
    for (let i = 0; i < this.RAIN_COUNT; i++) {
      this.rainPos[i * 3] = (Math.random() - 0.5) * 80;
      this.rainPos[i * 3 + 1] = Math.random() * 40;
      this.rainPos[i * 3 + 2] = 0.5 + Math.random() * 0.5; // alpha
      this.rainVel[i] = 30 + Math.random() * 20;
    }

    // Wind streak particles — horizontal lines
    this.windPos = new Float32Array(this.WIND_COUNT * 3);
    this.windVel = new Float32Array(this.WIND_COUNT);
    ...
    this.windMat = new THREE.ShaderMaterial({
      ...
      vertexShader: `
        ...
        void main() {
          vAlpha = position.z;
          vec4 mv = modelViewMatrix * vec4(position.xy, 0.0, 1.0);
          gl_PointSize = 5.0 * (220.0 / max(1.0, -mv.z));
          gl_Position = projectionMatrix * mv;
        }
      `,
      fragmentShader: `
        ...
        void main() {
          ...
          float d = length(p * vec2(0.2, 1.0));
          if (d > 0.5) discard;
          float a = smoothstep(0.5, 0.1, d) * vAlpha;
          gl_FragColor = vec4(0.7, 0.85, 1.0, a * 0.4);
        }
      `,
    });
    this.windPoints = new THREE.Points(windGeo, this.windMat);
    ...
    for (let i = 0; i < this.WIND_COUNT; i++) {
      this.windPos[i * 3] = (Math.random() - 0.5) * 80;
      this.windPos[i * 3 + 1] = (Math.random() - 0.5) * 30;
      this.windPos[i * 3 + 2] = 0.3 + Math.random() * 0.7; // alpha
      this.windVel[i] = 15 + Math.random() * 25;
    }
  }
  ...
  update(dt: number, time: number, bird: Bird, terrain: TerrainSystem, diving: boolean, ev: WeatherEvents): void {
    ...
    // Rain effect — active in storm biomes
    this.rainActive = biome.hazard === "storm";
    this.rainPoints.visible = this.rainActive;
    if (this.rainActive) {
      this.rainPoints.position.x = bird.x;
      this.rainPoints.position.y = bird.y;
      for (let i = 0; i < this.RAIN_COUNT; i++) {
        this.rainPos[i * 3 + 1] -= this.rainVel[i] * dt;
        if (this.rainPos[i * 3 + 1] < -20) {
          this.rainPos[i * 3] = (Math.random() - 0.5) * 80;
          this.rainPos[i * 3 + 1] = 20 + Math.random() * 10;
        }
      }
      (this.rainPoints.geometry.attributes.position as THREE.BufferAttribute).needsUpdate = true;
    }

    // Wind streak effect — active during gusts
    this.windActive = this.gust > 0.3;
    this.windPoints.visible = this.windActive;
    if (this.windActive) {
      this.windPoints.position.x = bird.x;
      this.windPoints.position.y = bird.y;
      const windDir = -1; // headwinds blow left
      for (let i = 0; i < this.WIND_COUNT; i++) {
        this.windPos[i * 3] += windDir * this.windVel[i] * this.gust * dt;
        if (this.windPos[i * 3] < -40 || this.windPos[i * 3] > 40) {
          this.windPos[i * 3] = windDir > 0 ? -40 : 40;
          this.windPos[i * 3 + 1] = (Math.random() - 0.5) * 30;
        }
      }
    ...
    // Lightning flash — random flashes during storms
    if (biome.hazard === "storm") {
      if (Math.random() < 0.02 * dt * 60 && this.stormCd <= 0) {
        this.lightningFlash = 1;
        if (ev.onLightning) ev.onLightning();
      }
    }
  ...
  dispose(): void {
    ...
    this.windPoints.geometry.dispose();
